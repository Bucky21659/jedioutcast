...
void StandardBotAI(bot_state_t *bs, float thinktime)
{
	...
	if (bs->currentEnemy)
	{
		if (BotGetWeaponRange(bs) == BWEAPONRANGE_SABER)
		{
			...
			if (bs->currentEnemy->health > 75 && bs->cur_ps.fd.forcePowerLevel[FP_SABERATTACK] > 2 &&
				bs->cur_ps.fd.saberAnimLevel != FORCE_LEVEL_3 && bs->saberPower)
			...
			else if (bs->currentEnemy->health > 40 && bs->cur_ps.fd.forcePowerLevel[FP_SABERATTACK] > 1 &&
				bs->cur_ps.fd.saberAnimLevel != FORCE_LEVEL_2)
			{ //they're down on health a little, use level 2 if we can
				Cmd_SaberAttackCycle_f(&g_entities[bs->client]);
			}
			else if (bs->cur_ps.fd.saberAnimLevel != FORCE_LEVEL_1)
