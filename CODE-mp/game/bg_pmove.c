...
qboolean PM_ForceJumpingUp(void)
{
	...
	if (PM_SaberInSpecial(pm->ps->saberMove))
	{
		return qfalse;
	}
...
qboolean PM_InDeathAnim( int anim )
{
	switch((anim&~ANIM_TOGGLEBIT))
	{
	case BOTH_DIVE1:
...
static qboolean PM_CheckJump( void ) 
{
	...
	if ( PM_InKnockDown( pm->ps ) || PM_InRoll( pm->ps, pm->ps->legsAnim ) ) 
	{//in knockdown
		return qfalse;		
	}
...
//							G_SoundOnEnt( pm->gent, CHAN_BODY, "sound/weapons/force/jump.wav" );
	...
	{//okay, we just jumped and we're in an attack
		if ( !PM_InRoll( pm->ps, pm->ps->legsAnim )
			...
			&& !PM_FlippingAnim( pm->ps->legsAnim ) //SAFASFASF
			&& !PM_SpinningAnim( pm->ps->legsAnim ) //SDGSDGGDSG
...
static void PM_CrashLand( void ) {
	...
	if (!PM_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->inAirAnim)
		...
		(pm->ps->legsAnim&~ANIM_TOGGLEBIT) == BOTH_WALL_RUN_RIGHT)
	{
		if (!PM_InRoll(pm->ps, pm->ps->legsAnim) && pm->ps->inAirAnim)
		{
			if (!PM_SaberInSpecial(pm->ps->saberMove) || pm->ps->weapon != WP_SABER)
			{
	...
	if ( pm->ps->pm_flags & PMF_DUCKED ) 
	{
		if( delta >= 2 && !PM_InOnGroundAnim( pm->ps->legsAnim ) && !PM_InKnockDown( pm->ps ) && !PM_InRoll(pm->ps, pm->ps->legsAnim) )
		{//roll!
			int anim = PM_TryRoll();
...
static void PM_Footsteps( void ) {
	...
	if ( pm->ps->pm_flags & PMF_DUCKED ) {
		...
		if ( PM_RunningAnim( pm->ps->legsAnim ) && !PM_InRoll(pm->ps, pm->ps->legsAnim) )
		{//roll!
			rolled = PM_TryRoll();
		}
		if ( !rolled )
...
//---------------------------------------
static void PM_DoChargingAmmoUsage( int *amount )
//---------------------------------------
{
// NOTENOTE No charging weapons yet.
/*
	int count;
	...
	if ( pm->ps->weapon == WP_BOWCASTER && pm->cmd.buttons & BUTTON_ALT_ATTACK )
	{
		...
		count = ( pm->cmd.serverTime - pm->ps->weaponChargeTime ) / BOWCASTER_CHARGE_UNIT;
		...
		if ( pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != -1 )
		{
			...
			if ( dif < 0 ) 
			{
				...
				pm->ps->weaponChargeTime = pm->cmd.serverTime - ( count * BOWCASTER_CHARGE_UNIT );
			}
		}	
	...
	else if ( pm->ps->weapon == WP_BRYAR_PISTOL && pm->cmd.buttons & BUTTON_ALT_ATTACK )
	{
		...
		count = ( pm->cmd.serverTime - pm->ps->weaponChargeTime ) / BRYAR_CHARGE_UNIT;
		...
		if ( pm->ps->ammo[ weaponData[pm->ps->weapon].ammoIndex ] != -1 )
		{
			...
			if ( dif < 0 ) 
			{
				...
				// now get a real chargeTime so the duplicated code in g_weapon doesn't get freaked
				pm->ps->weaponChargeTime = pm->cmd.serverTime - ( count * BRYAR_CHARGE_UNIT );
			}
		}	
...
static void PM_Weapon( void ) {
	...
	if (BG_InSpecialJump(pm->ps->legsAnim) ||
		PM_InRoll(pm->ps, pm->ps->legsAnim) ||
	...
	if (pm->ps->weapon == WP_SABER	/* && !cg.zoomMode */)		// WP_LIGHTSABER
	{	// Separate logic for lightsaber, but not when zoomed
...
//if(0)	//	if ( bg_itemlist[pm->ps->stats[STAT_HOLDABLE_ITEM]].giTag == HI_MEDKIT
	...
	if ( pm->cmd.buttons & BUTTON_GESTURE ) {
		if ( pm->ps->torsoTimer < 1 && pm->ps->forceHandExtend == HANDEXTEND_NONE &&
			...
			*/
			...
			pm->ps->forceDodgeAnim = BOTH_TALKGESTURE3;

			pm->ps->forceHandExtendTime = pm->cmd.serverTime + 2000;
...
//-------------------------------------------
{
	...
	/*if ( pm->ps->weapon == WP_SABER && !pm->ps->zoomMode)
	{
	...
	if ( pm->ps->weapon == WP_DISRUPTOR)
	{
		...
		if ( pm->cmd.buttons & BUTTON_ATTACK )
		{
			if ( pm->ps->weapon == WP_DISRUPTOR && pm->ps->zoomTime+1500 > pm->cmd.serverTime)
			{ //make it so we can't fire in zoomed mode until we've been zoomed for a moment
				pm->cmd.buttons &= ~BUTTON_ATTACK;
			}
...
void BG_AdjustClientSpeed(playerState_t *ps, usercmd_t *cmd, int svTime)
{
	...
	if ( PM_SaberInAttack( ps->saberMove ) && cmd->forwardmove < 0 )
	...
	else if ( ps->weapon == WP_SABER && PM_SaberInAttack( ps->saberMove ) )
	...
	if ( PM_InRoll( ps, ps->legsAnim ) && ps->speed > 200 )
...
//	if (pm->ps->groundEntityNum == ENTITYNUM_NONE || pml.groundTrace.fraction == 1)
//	{ //No holding jump in air to jump upon land
//		pm->cmd.upmove = 0;
//		pm->ps->pm_flags |= PMF_JUMP_HELD;
//	}
...
//		PM_UpdateGhoul2AnimFromState();
	...
	// torso animation
//  rww - This didn't seem to do anything at all..
//	PM_TorsoAnimation();
