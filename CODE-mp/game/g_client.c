...
#define JMSABER_RESPAWN_TIME 60000 //in case it gets stuck somewhere no one can reach
...
void JMSaberTouch(gentity_t *self, gentity_t *other, trace_t *trace)
{
	...
	{ //full health when you become the Jedi Master
		other->client->ps.stats[STAT_HEALTH] = other->health = 100;
	}
...
//	areabits = client->areabits;
	...
	if (selFP && selFP[0])
	{
		client->sess.selectedFP = atoi(selFP)+1;
	}
	selFP = Info_ValueForKey (userinfo, "sal"); // "sal" == "saber attack level"
	if (selFP && selFP[0])
	{
		client->sess.saberLevel = atoi(selFP)+1;
	}
