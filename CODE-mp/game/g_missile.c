...
void G_MissileImpact( gentity_t *ent, trace_t *trace ) {
	...
	*/
	...
	{ //only block one projectile per 200ms (to prevent giant swarms of projectiles being blocked)
		...
		if (other->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_1)
		{
			//if def is only level 1, instead of deflecting the shot it should just die here
		}
		else if (other->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_2)
		{
		...
		other->client->ps.saberBlockTime = level.time + (350 - (other->client->ps.fd.forcePowerLevel[FP_SABERDEFEND]*100)); //200;

		if (other->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_3)
		{
	...
	{ //hit this person's saber, so..
		...
		{ //for now still deflect even if saberBlockTime >= level.time because it hit the actual saber
			...
			if (otherOwner->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_1)
			{
				//if def is only level 1, instead of deflecting the shot it should just die here
			}
			else if (otherOwner->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_2)
			{
			...
			otherOwner->client->ps.saberBlockTime = level.time + (350 - (otherOwner->client->ps.fd.forcePowerLevel[FP_SABERDEFEND]*100));//200;

			if (otherOwner->client->ps.fd.forcePowerLevel[FP_SABERDEFEND] == FORCE_LEVEL_3)
			{
